...
export default function OperationsPlanner({ language }) {
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
              ...
              <strong style={{ color: 'var(--amber-400)', fontSize: '0.92rem' }}>
                {t.disruption_detected}: {replanMsg.trigger_rainfall_mm}mm Rain
              </strong>
            </div>
          ...
          {schedule.map((task) => {
            ...
            return (
              <div 
                key={task.task_id}
                style={{
                  background: isRescheduled ? 'rgba(245, 158, 11, 0.08)' : 'var(--card-bg)',
                  border: isRescheduled ? '1px solid rgba(245, 158, 11, 0.35)' : '1px solid var(--border-glass)',
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                      <span className={`badge ${isCritical ? 'badge-rose' : (isHigh ? 'badge-amber' : 'badge-emerald')}`} style={{ fontSize: '0.68rem' }}>
                        {task.priority} {t.priority_label}
                      </span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '14px', fontSize: '0.78rem', color: 'var(--text-muted)', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <User size={13} /> {t.assigned}: {task.assigned_worker}
                      </span>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Tractor size={13} /> {t.tool}: {task.assigned_equipment}
                      </span>
                    </div>

                    {task.fertilizer_dosage && task.fertilizer_dosage !== 'None' && (
                      <span style={{ fontSize: '0.76rem', color: 'var(--emerald-400)', display: 'block', marginTop: '4px', fontWeight: 600 }}>
                        {t.input_recipe}: {task.fertilizer_dosage}
                      </span>
                    )}
                  </div>
                </div>
